...
import { TeamData, PlayerData, StatTypeData } from './useNormalizationData';

type Entity = TeamData | PlayerData | StatTypeData;
...
export function useEntitySearch<T extends Entity>(
  ...
  useEffect(() => {
    ...
    }, entities.length > 500 ? 300 : 0); // Instant for small lists, 300ms for large
  ...
  const filteredEntities = useMemo(() => {
    ...
    // 1. Sport Filter (common to all, though StatTypes might have different logic handled by caller, 
    // but usually they filter BEFORE passing here? No, let's handle it here if passed)
    if (sportFilter !== "All") {
      result = result.filter((e) => { 
          // Check if entity has 'sport' property. StatTypes might not? 
          // Checking the types: TeamData (sport), PlayerData (sport), StatTypeData (no sport).
    ...
    if (debouncedQuery.trim()) {
      ...
      result = result.filter((e) => {
        ...
        if ('abbreviations' in e && (e as any).abbreviations) {
            if ((e as any).abbreviations.some((a: string) => a.toLowerCase().includes(q))) return true;
        }
