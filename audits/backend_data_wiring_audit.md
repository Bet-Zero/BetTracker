...
|----|----------------------------|---------------------------|-------------------|----------|---------------|
| PG-1 | `computeNetNumeric` (betToFinalRows) returns `undefined` for pending (display blank); `getNetNumeric` (displaySemantics) returns `0` for pending (KPI totals) | KPI and display net semantics are intentionally different for pending bets. The drift risk is NOT “KPI vs table totals” — the drift risk is that either function changes its intended settled-bet behavior without tests catching it. | Add invariant tests that: (1) KPI net totals always equal `sum(bets.map(getNetNumeric))` for any dataset; (2) Display net for pending is always blank (undefined); (3) For settled bets, display net matches `payout - stake` formatting expectations. **Never reconcile KPI totals against FinalRows.** | `parsing/shared/betToFinalRows.ts:648-695`, `services/displaySemantics.ts:67-74` | `src/tests/invariants.test.ts`: KPI totals reconcile only against `getNetNumeric`; `parsing/tests/betToFinalRows.test.ts`: pending display blank + settled display formatting |
...
| **INV-10:** All time logic must be timezone-stable. `bet.placedAt` MUST be an ISO 8601 timestamp with an explicit timezone (`Z` or `±HH:MM`). All comparisons MUST be done using epoch milliseconds (`new Date(placedAt).getTime()`), never by string comparison. Fixtures MUST include timezone-bearing `placedAt` values to prevent local/UTC drift. | Unit test needed | `filterPredicates.ts`, `DashboardView.tsx:calculateNetForPeriod()` |
