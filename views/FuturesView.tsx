      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {/* Status Badge */}
            <span className={`px-2 py-0.5 rounded text-xs font-medium ${statusColors[position.status]}`}>
...
const FuturesView: React.FC = () => {
  ...
  const [hedgePosition, setHedgePosition] = useState<FuturesPosition | null>(null);
  ...
  const futuresData = useMemo(() => {
    ...
    const positions: FuturesPosition[] = Array.from(positionMap.entries()).map(([key, positionBets]) => {
      const [entity, futuresType, sport] = key.split('__');
      const totalStake = positionBets.reduce((sum, b) => sum + b.stake, 0);
      const totalPotentialPayout = positionBets.reduce((sum, b) => sum + b.payout, 0);
      const resolutionDate = estimateResolutionDate(futuresType, sport);
    ...
    const totalExposure = pendingPositions.reduce((sum, p) => sum + p.totalStake, 0);
    const totalPotentialPayout = pendingPositions.reduce((sum, p) => sum + p.totalPotentialPayout, 0);
  ...
  const displayPositions = useMemo(() => {
    ...
    let positions = viewMode === 'history' 
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-lg shadow-md p-4">
          ...
          <p className="text-2xl font-bold text-neutral-900 dark:text-white mt-1">
            {futuresData.settledStats.wins}W - {futuresData.settledStats.losses}L
          </p>
          <p className={`text-xs mt-1 ${futuresData.settledStats.net >= 0 ? 'text-green-500' : 'text-red-500'}`}>
            {futuresData.settledStats.net >= 0 ? '+' : ''}{formatCurrency(futuresData.settledStats.net)} net
          </p>
        </div>
              ...
              >
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-medium text-neutral-700 dark:text-neutral-300">
                      {formatCurrency(position.totalStake)} → {formatCurrency(position.totalPotentialPayout)}
                    </p>
