...
export function classifyBet(
  bet: Omit<Bet, "id" | "marketCategory" | "raw" | "tail">,
): MarketCategory {
  // Input validation
  ...
  if (!bet.betType) {
    console.warn(
      `[classifyBet] Missing betType for bet ${bet.betId || "unknown"}`,
    );
  }
...
export function classifyLeg(market: string, sport: string): string {
  ...
  if (!sport || typeof sport !== "string" || sport.trim() === "") {
    console.warn(
      "[classifyLeg] Invalid or empty sport provided, defaulting to NBA",
    );
    sport = "NBA";
  }
...
export function determineType(
  ...
  if (typeof market !== "string") {
    console.warn(
      "[determineType] Invalid market type provided:",
      typeof market,
    );
    return "";
  }
  ...
  if (typeof category !== "string") {
    console.warn(
      "[determineType] Invalid category type provided:",
      typeof category,
    );
    return "";
  }
...
export function abbreviateMarket(market: string, sport?: string): string {
  ...
  const normalized = normalizeBetType(market, sport as Sport);

  // Check if there's a custom display name configured
  const betTypeInfo = getBetTypeInfo(normalized, sport as Sport);
  if (betTypeInfo?.displayName) {
    return betTypeInfo.displayName;
  }

  // If we have sport context and the normalized name starts with the sport,
  // strip the sport prefix for cleaner display (e.g., "NBA MVP" -> "MVP")
  if (sport && normalized.startsWith(sport + " ")) {
    return normalized.substring(sport.length + 1);
  }
