...
export const parseDraftKingsHTML = (html: string): Bet[] => {
  ...
  betCards.forEach((card) => {
    try {
        ...
        const betTypeSubtitles = card.querySelectorAll('span[data-test-id^="bet-details-subtitle-"]');
        let typeText = '';
        if (betTypeSubtitles.length > 0) {
            typeText = betTypeSubtitles[0].textContent?.toLowerCase() || '';
        }

        let isParlay = typeText.includes('parlay') || typeText.includes('sgp') || typeText.includes('+');

        const context = { element: card, header, footer };

        const bet = isParlay
            ? parseParlayBet(context)
            : parseSingleBet(context);
